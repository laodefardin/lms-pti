<div class="fade-in">
    ...
    <div style="display:grid;grid-template-columns:300px 1fr;gap:1.5rem;align-items:start;">
        ...
        <div style="display:flex;flex-direction:column;gap:1.5rem;">
            ...
            <div class="card" style="text-align:center;padding:2rem 1.5rem;">
                <div style="position:relative;width:110px;height:110px;margin:0 auto 1.25rem;">
                    ...
                    @else
                        <img src="{{ $user->foto ? asset('storage/'.$user->foto) : $user->foto_url_attribute ?? 'https://ui-avatars.com/api/?name='.urlencode($user->name).'&color=004b93&background=e8f2fc&size=200' }}"
                    ...
                    {{-- Upload overlay --}}
                    <label for="foto-upload" style="position:absolute;bottom:0;right:0;width:32px;height:32px;border-radius:50%;background:var(--teal);border:2px solid var(--bg-card);display:flex;align-items:center;justify-content:center;cursor:pointer;transition:transform .2s;"
                ...
                <span style="display:inline-flex;align-items:center;gap:.4rem;padding:.3rem .9rem;border-radius:99px;font-size:.75rem;font-weight:700;text-transform:uppercase;letter-spacing:.04em;background:rgba(0,75,147,.12);color:#004b93;">
                    <i class="fas fa-chalkboard-teacher"></i> Dosen
                </span>

                {{-- Google badge jika terhubung --}}
                @if($user->google_id)
            ...
            <div class="card" style="padding:1.25rem;">
                <p style="font-size:.72rem;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:var(--text-muted);margin-bottom:1rem;">Statistik Mengajar</p>
                <div style="display:flex;flex-direction:column;gap:.75rem;">
                    <div style="display:flex;justify-content:space-between;align-items:center;">
                        <span style="font-size:.85rem;color:var(--text-secondary);"><i class="fas fa-door-open" style="width:16px;color:var(--teal);"></i> Total Kelas</span>
                        <strong style="color:var(--text-primary);">{{ $stats['total_kelas'] }}</strong>
                    </div>
                    <div style="display:flex;justify-content:space-between;align-items:center;">
                        <span style="font-size:.85rem;color:var(--text-secondary);"><i class="fas fa-users" style="width:16px;color:var(--teal);"></i> Total Mahasiswa</span>
                        <strong style="color:var(--text-primary);">{{ $stats['total_mahasiswa'] }}</strong>
                    </div>
                    <div style="display:flex;justify-content:space-between;align-items:center;">
                        <span style="font-size:.85rem;color:var(--text-secondary);"><i class="fas fa-tasks" style="width:16px;color:var(--teal);"></i> Total Tugas</span>
                        <strong style="color:var(--text-primary);">{{ $stats['total_tugas'] }}</strong>
                    </div>
                </div>
            </div>
            ...
            <div class="card" style="padding:1.25rem;">
                ...
                <div style="display:flex;flex-direction:column;gap:.75rem;">
                    <div>
                        <p style="font-size:.72rem;color:var(--text-muted);margin-bottom:.15rem;">NIP / NIDN</p>
                        <p style="font-size:.9rem;font-weight:600;color:var(--text-primary);">{{ $user->nim_nidn ?? '-' }}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        <p style="font-size:.72rem;color:var(--text-muted);margin-bottom:.15rem;">Bergabung</p>
        ...
        <div style="display:flex;flex-direction:column;gap:1.5rem;">
            ...
            <div class="card" style="padding:1.75rem;">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.5rem;padding-bottom:1rem;border-bottom:1px solid var(--border);">
                    <div>
                        ...
                        <p style="font-size:.8rem;color:var(--text-muted);">Perbarui nama, nomor HP, dan bio Anda.</p>
                    </div>
                    @if(!$editMode)
                ...
                @if($editMode)
                    <form wire:submit.prevent="save" style="display:flex;flex-direction:column;gap:1.25rem;">
                        ...
                        <div>
                            ...
                            <input type="email" value="{{ $email }}" class="form-input" disabled style="opacity:.6;cursor:not-allowed;" title="Email tidak dapat diubah">
                            <p style="font-size:.72rem;color:var(--text-muted);margin-top:.25rem;"><i class="fas fa-lock"></i> Email tidak dapat diubah melalui halaman ini.</p>
                        </div>
                        <div>
                        ...
                        <div>
                            <label style="font-size:.8rem;font-weight:600;color:var(--text-secondary);display:block;margin-bottom:.4rem;">Bio / Tentang Saya</label>
                            <textarea wire:model="bio" rows="3" class="form-input" placeholder="Ceritakan sedikit tentang Anda..."></textarea>
                ...
                @else
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.25rem;">
                        ...
                        <div>
                            <p style="font-size:.75rem;color:var(--text-muted);margin-bottom:.2rem;">NIP / NIDN</p>
                            <p style="font-weight:600;color:var(--text-primary);">{{ $user->nim_nidn ?? '-' }}</p>
                        </div>
                        <div>
            ...
            {{-- Ubah Password Card --}}
            <div class="card" style="padding:1.75rem;">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:{{ $showPasswordForm ? '1.5rem' : '0' }};{{ $showPasswordForm ? 'padding-bottom:1rem;border-bottom:1px solid var(--border);' : '' }}">
                    <div>
                ...
                @if($showPasswordForm)
                    <form wire:submit.prevent="changePassword" style="display:flex;flex-direction:column;gap:1.25rem;">
                        ...
                        <div>
                            <label style="font-size:.8rem;font-weight:600;color:var(--text-secondary);display:block;margin-bottom:.4rem;">Password Baru</label>
                            <input type="password" wire:model="newPassword" class="form-input" placeholder="Minimal 8 karakter">
                        ...
                        <div>
                            ...
                            <input type="password" wire:model="newPasswordConfirmation" class="form-input" placeholder="Ulangi password baru">
                        </div>
                        <div style="display:flex;gap:.75rem;">
                            <button type="submit" class="btn btn-primary" wire:loading.attr="disabled">
