<div class="fade-in">
    ...
    @if($myRank)
        <div class="card" style="margin-bottom: 2rem; background: linear-gradient(135deg, var(--teal), var(--teal-dark)); color: white; display: flex; justify-content: space-between; align-items: center;">
            <div>
            ...
            <div style="text-align: right;">
                ...
                <div style="opacity: 0.9;">{{ $myRank->gamifikasi_poin_sum_jumlah_poin ?? 0 }} Poin</div>
            </div>
        </div>
    @endif
        ...
        <div style="display: flex; justify-content: center; align-items: flex-end; gap: 1rem; margin-bottom: 3rem; padding-top: 2rem;">
            ...
            @if(isset($top3[1]))
                <div style="display: flex; flex-direction: column; align-items: center; flex: 1; max-width: 150px;">
                    ...
                    <div class="badge badge-gray" style="margin-bottom: 0.5rem;">{{ $top3[1]->gamifikasi_poin_sum_jumlah_poin ?? 0 }} pts</div>
            ...
            @if(isset($top3[0]))
                <div style="display: flex; flex-direction: column; align-items: center; flex: 1; max-width: 150px;">
                    ...
                    <div class="badge badge-orange" style="margin-bottom: 0.5rem;">{{ $top3[0]->gamifikasi_poin_sum_jumlah_poin ?? 0 }} pts</div>
            ...
            @if(isset($top3[2]))
                <div style="display: flex; flex-direction: column; align-items: center; flex: 1; max-width: 150px;">
                    ...
                    <div class="badge badge-gray" style="background: #CD7F32; color: white; border:none; margin-bottom: 0.5rem;">{{ $top3[2]->gamifikasi_poin_sum_jumlah_poin ?? 0 }} pts</div>
                    ...
                    <tbody>
                        @foreach($others as $mhs)
                            <tr style="{{ $mhs->id === auth()->id() ? 'background: var(--teal-dim); border-left: 4px solid var(--teal);' : 'border-bottom: 1px solid var(--border);' }}">
                                ...
                                <td style="padding: 1rem; font-weight: bold; color: var(--teal);">{{ $mhs->gamifikasi_poin_sum_jumlah_poin ?? 0 }}</td>
                                <td style="padding: 1rem;">
                                    <span class="badge badge-teal">Tingkat {{ max(1, floor(($mhs->gamifikasi_poin_sum_jumlah_poin ?? 0) / 100)) }}</span>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
